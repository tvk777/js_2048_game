...
"use strict";
/* eslint no-console: ["error", { allow: ["warn", "log"] }] */ const Game = require("789b00406e138236");
const game = new Game();
const startButton = document.querySelector(".start");
startButton.addEventListener("click", (e)=>{
    if (startButton.classList.contains("start")) game.start();
    if (startButton.classList.contains("restart")) game.restart();
   ...
   */ restart() {
    ...
    getEmptyCells() {
        ...
        for(let row = 0; row < this.size; row++){
            for(let cell = 0; cell < this.size; cell++)if (this.board[row][cell] === 0) emptyCells.push({
                row,
                cell
            });
        }
    ...
    checkStatus() {
        ...
        if (this.getEmptyCells().length === 0 && !this.canMerge()) this.status = "lose";
        return this.status;
    }
}
module.exports = Game;
